...
class ProductTest < ActiveSupport::TestCase
	...
	test "product title must greater or equal to 10 chars" do
		# длина названия товара должна быть больше либо равна 10
		product = Product.new(description: "yyy",
		image_url: "zzz.jpg",
		price: 100)
		product.title = "a"
		assert product.invalid?
		assert_equal "product title must greater or equal to 10 chars",
		product.errors[:title].join('; ')
		product.title = "aaaaaaaaaa"
		assert product.valid?
		product.title = "aaaaaaaaaaaaa"
		assert product.valid?
	end
end
